...
3. 已迁移并验证部分组件使用 CSS Variables，包括 `Button`、`Input`、`Message`、`Tag`、`Progress`、`Card`、`Table`、`Form`、`Upload`、`Menu`、`Pagination`、`Timeline`。
4. 已在 Storybook 中新增 `主题/Theme`、Button/Table/Form/Upload/Menu/Pagination/Timeline/Card 企业主题示例，用于对比默认主题、企业级主题和组件级覆盖。
...
继续迁移剩余组件时，优先评估是否只需复用现有 global/semantic token。只有组件独有视觉语义稳定后，再考虑新增组件级 token。
