...
const meta: Meta<typeof Icon> = {
  ...
  argTypes: {
    theme: {
      control: { type: 'select' },
      options: ['primary', 'secondary', 'success', 'danger', 'warning', 'info', 'light', 'dark'] as IconTheme[],
      description: '图标主题颜色',
    },
    size: {
      control: { type: 'select' },
...
export const Themes: StoryObj<typeof Icon> = {
  render: () => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '32px', padding: '24px' }}>
      {THEMES.map(({ theme, bg }) => (
        <div
          key={theme as string}
          style={{
        ...
        >
          ...
          <div style={{ marginTop: '10px', fontSize: '12px', color: bg ? '#fff' : 'inherit', fontWeight: 500 }}>
            {theme}
          </div>
        </div>
      ))}
...
export const Sizes: StoryObj<typeof Icon> = {
  render: () => (
    <div style={{ display: 'flex', gap: '32px', alignItems: 'flex-end', padding: '24px', flexWrap: 'wrap' }}>
      {SIZES.map(size => (
        <div key={size} style={{ textAlign: 'center' }}>
          <Icon icon={faUser} size={size} theme="primary" />
          <div style={{ marginTop: '10px', fontSize: '12px', fontWeight: 500 }}>{size}</div>
        </div>
      ))}
    </div>
  ),
...
const STATUS_ICONS = [
  { icon: faCircleCheck, theme: 'success' as IconTheme, label: '成功', color: '#4caf50' },
  { icon: faCircleXmark, theme: 'danger' as IconTheme, label: '失败', color: '#f44336' },
  { icon: faTriangleExclamation, theme: 'warning' as IconTheme, label: '警告', color: '#ffc107' },
  { icon: faCircleInfo, theme: 'info' as IconTheme, label: '信息', color: '#2196f3' },
];

export const StatusIcons: StoryObj<typeof Icon> = {
  render: () => (
    <div style={{ display: 'flex', gap: '40px', padding: '24px', alignItems: 'center' }}>
      {STATUS_ICONS.map(({ icon, theme, label, color }) => (
        <div key={label} style={{ textAlign: 'center' }}>
          <Icon icon={icon} theme={theme} size="3x" />
          <div style={{ marginTop: '10px', fontSize: '13px', color, fontWeight: 600 }}>{label}</div>
        </div>
      ))}
    </div>
  ),
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '24px', padding: '24px' }}>
      {COMMON_ICONS.map(({ icon, label }) => (
        ...
        >
          ...
          <div style={{ marginTop: '10px', fontSize: '10px', color: '#616161' }}>{label}</div>
        </div>
      ))}
    </div>
  ),
